<!-- src/views/node/wholBatch/list.vue -->
<!-- 流通节点端：批发商批号管理列表页面 -->
<!-- UI布局、配色、交互逻辑完全对齐冷冻加工批号管理 processBatch/list.vue -->
<!-- 业务状态说明：state=1【新建待发布】｜state=2【已发布上架】 -->
<template>
  <div class="whol-batch-list">
    <!-- 顶部标题返回栏，固定高度不滚动 -->
    <div class="page-header">
      <div class="back-icon" @click="$router.back()"></div>
      <div class="title">批发商产品批号管理</div>
    </div>
    ...
    <div class="scroll-container">
      ...
      <div class="section">
        ...
        <!-- 空数据提示 -->
        <div v-if="draftList.length === 0" class="empty-tip">暂无待发布的批发商批号</div>

        <!-- 待发布批号卡片循环渲染 -->
        <div class="batch-card" v-for="item in draftList" :key="item.wbId">
          <div class="card-row">
            <span class="label">批发商业务批号：</span>
      ...
      <div class="section">
        ...
        <!-- 空数据提示 -->
        <div v-if="publishedList.length === 0" class="empty-tip">暂无已发布的批发商批号</div>
        ...
        <div
          ...
          :key="item.wbId"
          :class="{ selected: selectedIds.includes(item.wbId) }"
          @click="toggleSelect(item.wbId)"
        >
          <div class="select-row">
            <span class="checkbox" :class="{ checked: selectedIds.includes(item.wbId) }"></span>
    ...
    <div class="tab-bar">
      ...
      <div class="tab-item active" @click="$router.push('/node/wholBatch/list')">
    ...
    <div v-if="editDialogVisible" class="dialog-mask" @click.self="editDialogVisible = false">
      <div class="dialog-box">
        <div class="dialog-title">更新批发商产品批号</div>
        <div class="form-item">
          <label class="form-label">批发商业务批号 <span class="required">*</span></label>
          <input v-model="editForm.batchId" class="form-input" placeholder="请输入批发商业务批号" />
        </div>
        <div class="form-item">
          <label class="form-label">上游冷冻加工企业ID</label>
          <input v-model="editForm.sourceNodeId" type="number" class="form-input" placeholder="上游加工企业主键ID" />
        </div>
        <div class="form-item">
          <label class="form-label">上游加工成品批号ID</label>
          <input v-model="editForm.sourceBatchId" type="number" class="form-input" placeholder="上游成品批号主键ID" />
        </div>
        <div class="form-item">
        ...
        <div class="form-item">
          ...
          <input v-model="editForm.reportId" class="form-input" placeholder="填写质检报告编号" />
        </div>
        <div class="form-item">
          <label class="form-label">检验人员名称</label>
          <input v-model="editForm.testName" class="form-input" placeholder="填写质检工作人员姓名" />
        </div>
        <div class="form-item">
    ...
    <div v-if="detailDialogVisible" class="dialog-mask" @click.self="detailDialogVisible = false">
      <div class="dialog-box">
        <div class="dialog-title">批发商批号详情</div>
        <div class="detail-row">
          <span class="detail-label">批发商业务批号：</span>
          <span>{{ detailForm.batchId || '-' }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">上游冷冻加工企业ID：</span>
          <span>{{ detailForm.sourceNodeId || '-' }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">上游成品批号ID：</span>
        ...
        <div class="detail-row">
          <span class="detail-label">质检报告编号：</span>
          <span>{{ detailForm.reportId || '-' }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">检验人员名称：</span>
...
import {
  ...
  deleteWholBatch,
  offShelveWholBatch,
  sendWholConfirmRequest
} from '@/api/node/wholBatch'
...
const loadBatchList = async () => {
  const res = await getWholBatchList()
  if (res.code === 200) {
...
const submitEdit = async () => {
  if (!editForm.value.batchId) {
    ElMessage.warning('批发商业务批号不能为空')
    return
  }
  const res = await updateWholBatch(editForm.value)
  if (res.code === 200) {
...
/**
 * 删除单条批发商批号
 * @param {Number} wbId 批发商批号主键wbId
 */
const handleDelete = async (wbId) => {
  ElMessageBox.confirm('确认删除这条批发商批号？删除后不可恢复', '提示', {
    confirmButtonText: '确认删除',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    await deleteWholBatch(wbId)
...
/**
 * 向下游零售商发送进场确认请求
 * @param {Number} wbId 批发商批号主键
 */
const handleSendConfirmReq = async (wbId) => {
  ElMessageBox.confirm('确认向下游发送进场确认请求？', '提示').then(async () => {
    const res = await sendWholConfirmRequest(wbId)
    if (res.code === 200) {
      ElMessage.success('进场请求发送成功，等待下游企业确认')
      loadBatchList()
    }
  })
}
...
/**
 * 勾选/取消勾选批号主键，用于批量下架
 * @param {Number} wbId 批号主键
 */
const toggleSelect = (wbId) => {
  const idx = selectedIds.value.indexOf(wbId)
  if (idx > -1) {
    selectedIds.value.splice(idx, 1)
  } else {
    selectedIds.value.push(wbId)
  }
}
...
const handleBatchOffShelve = () => {
  ...
  ElMessageBox.confirm(`确认下架选中的 ${selectedIds.value.length} 个批发商批号？`, '提示').then(async () => {
    try {
      for (const id of selectedIds.value) {
        await offShelveWholBatch(id)
      }
