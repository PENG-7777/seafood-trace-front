...
export function deleteFishBatch(fbId) {
  return nodeRequest({
    url: `/api/node/fishBatch/delete/${fbId}`,
    method: 'DELETE'
  })
}
...
export function offShelveFishBatch(fbId) {
  return nodeRequest({
    url: `/api/node/fishBatch/offShelve/${fbId}`,
    method: 'PUT'
  })
}
 ...
 * @returns {Promise} Result，成功无返回data
 * PUT路径传参
 * 接口地址：/api/node/fishBatch/confirm/{processBatchId}
 */
export function confirmProcessBatch(processBatchId) {
  return nodeRequest({
    url: `/api/node/fishBatch/confirm/${processBatchId}`,
    method: 'PUT'
  })
}
